#!/usr/bin/env python3
"""Create a localized Khmer edge-softening defect for visual calibration."""
...
def blur_line_region(
    ...
    *,
        ...
        for y in range(top, bottom + 1):
            for x in range(region_left, region_right + 1):
                ...
                for channel in range(3):
                    current[offset + channel] = round(
                        (
                            previous[offset - 3 + channel]
                            + 2 * previous[offset + channel]
                            + previous[offset + 3 + channel]
                        )
                        / 4
                    )
    ...
    for y in range(top, bottom + 1):
        for x in range(region_left, region_right + 1):
            offset = (y * width + x) * 3
            if current[offset : offset + 3] != source_pixels[offset : offset + 3]:
                changed_pixels += 1

    if changed_pixels == 0:
        raise RuntimeError("edge-softening mutation changed no pixels")
...
def main() -> int:
    ...
    variant = f"sensitivity-blur-w{width_label}-xp{x_label}-p{args.passes}"
    candidate_prefix = f"system-harfbuzz-blur-w{width_label}-xp{x_label}-p{args.passes}"
    ...
    result_path = directory / f"sensitivity-results-blur-w{width_label}-xp{x_label}-p{args.passes}.json"
