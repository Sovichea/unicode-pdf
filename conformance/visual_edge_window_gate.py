#!/usr/bin/env python3
"""Gate localized Khmer contour fidelity with overlapping line windows.

Area overlap can remain deceptively high when a small mark or stroke contour moves.
This gate extracts the binary ink boundary in each overlapping text-line window and
scores contour agreement with a tolerance-aware boundary F1 metric. The metric is
therefore focused on glyph/stroke geometry rather than page background or fill area.
"""

from __future__ import annotations

import argparse
import json
from pathlib import Path
import sys

from visual_backend_parity import pixel_is_ink, read_ppm
from visual_line_gate import detect_ink_bands, numbered_pages


def boundary_pixels(mask: list[bool], width: int, height: int) -> set[tuple[int, int]]:
    boundary: set[tuple[int, int]] = set()
    for y in range(height):
        for x in range(width):
            if not mask[y * width + x]:
                continue
            if x == 0 or y == 0 or x == width - 1 or y == height - 1:
                boundary.add((x, y))
                continue
            if (
                not mask[y * width + x - 1]
                or not mask[y * width + x + 1]
                or not mask[(y - 1) * width + x]
                or not mask[(y + 1) * width + x]
            ):
                boundary.add((x, y))
    return boundary


def matched_fraction(
    source: set[tuple[int, int]],
    target: set[tuple[int, int]],
    tolerance: int,
) -> float:
    if not source:
        return 1.0 if not target else 0.0
    matched = 0
    for x, y in source:
        found = False
        for dy in range(-tolerance, tolerance + 1):
            for dx in range(-tolerance, tolerance + 1):
                if (x + dx, y + dy) in target:
                    found = True
                    break
            if found:
                break
        matched += int(found)
    return matched / len(source)


def boundary_f1(
    reference_mask: list[bool],
    candidate_mask: list[bool],
    width: int,
    height: int,
    tolerance: int,
) -> tuple[float, int, int]:
    reference_boundary = boundary_pixels(reference_mask, width, height)
    candidate_boundary = boundary_pixels(candidate_mask, width, height)
    precision = matched_fraction(candidate_boundary, reference_boundary, tolerance)
    recall = matched_fraction(reference_boundary, candidate_boundary, tolerance)
    if precision + recall == 0.0:
        score = 0.0
    else:
        score = 2.0 * precision * recall / (precision + recall)
    return score, len(reference_boundary), len(candidate_boundary)


def compare_windows(
    reference_path: Path,
    candidate_path: Path,
    *,
    window_fraction: float,
    stride_fraction: float,
    ink_threshold: int,
    min_window_ink_pixels: int,
    max_blank_row_gap: int,
    edge_tolerance: int,
) -> dict[str, object]:
    width, height, reference = read_ppm(reference_path)
    candidate_width, candidate_height, candidate = read_ppm(candidate_path)
    if (width, height) != (candidate_width, candidate_height):
        raise RuntimeError(
            "render dimensions differ: "
            f"reference={width}x{height}, candidate={candidate_width}x{candidate_height}"
        )

    bands = detect_ink_bands(
        width,
        height,
        reference,
        candidate,
        ink_threshold=ink_threshold,
        max_blank_row_gap=max_blank_row_gap,
    )
    windows: list[dict[str, int | float]] = []
    for line_index, (top, bottom) in enumerate(bands, start=1):
        xs: list[int] = []
        for y in range(top, bottom + 1):
            for x in range(width):
                offset = (y * width + x) * 3
                if pixel_is_ink(reference, offset, ink_threshold) or pixel_is_ink(
                    candidate, offset, ink_threshold
                ):
                    xs.append(x)
        if not xs:
            continue

        left, right = min(xs), max(xs)
        ink_width = right - left + 1
        window_width = max(1, round(ink_width * window_fraction))
        stride = max(1, round(ink_width * stride_fraction))
        starts = list(range(left, max(left + 1, right - window_width + 2), stride))
        final_start = max(left, right - window_width + 1)
        if not starts or starts[-1] != final_start:
            starts.append(final_start)

        for window_index, window_left in enumerate(starts, start=1):
            window_right = min(right, window_left + window_width - 1)
            local_width = window_right - window_left + 1
            local_height = bottom - top + 1
            reference_mask: list[bool] = []
            candidate_mask: list[bool] = []
            ink_union_pixels = 0
            for y in range(top, bottom + 1):
                for x in range(window_left, window_right + 1):
                    offset = (y * width + x) * 3
                    reference_ink = pixel_is_ink(reference, offset, ink_threshold)
                    candidate_ink = pixel_is_ink(candidate, offset, ink_threshold)
                    reference_mask.append(reference_ink)
                    candidate_mask.append(candidate_ink)
                    ink_union_pixels += int(reference_ink or candidate_ink)

            if ink_union_pixels < min_window_ink_pixels:
                continue
            score, reference_edges, candidate_edges = boundary_f1(
                reference_mask,
                candidate_mask,
                local_width,
                local_height,
                edge_tolerance,
            )
            windows.append(
                {
                    "line": line_index,
                    "window": window_index,
                    "left": window_left,
                    "top": top,
                    "right": window_right,
                    "bottom": bottom,
                    "ink_union_pixels": ink_union_pixels,
                    "reference_boundary_pixels": reference_edges,
                    "candidate_boundary_pixels": candidate_edges,
                    "boundary_f1": score,
                }
            )

    if not windows:
        raise RuntimeError("no edge window met the minimum rendered-ink threshold")

    return {
        "line_count": len(bands),
        "windows": windows,
        "minimum_window_boundary_f1": min(
            float(window["boundary_f1"]) for window in windows
        ),
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dir", required=True)
    parser.add_argument("--reference-prefix", default="system-harfbuzz")
    parser.add_argument("--candidate-prefix", default="harfrust")
    parser.add_argument("--window-fraction", type=float, default=0.05)
    parser.add_argument("--stride-fraction", type=float, default=0.025)
    parser.add_argument("--ink-threshold", type=int, default=250)
    parser.add_argument("--min-window-ink-pixels", type=int, default=32)
    parser.add_argument("--max-blank-row-gap", type=int, default=2)
    parser.add_argument("--edge-tolerance", type=int, default=1)
    parser.add_argument("--min-window-boundary-f1", type=float, default=0.90)
    parser.add_argument("--expect-below", type=float, default=None)
    parser.add_argument("--output", default=None)
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    if not 0.0 < args.window_fraction <= 1.0:
        raise RuntimeError("--window-fraction must be in (0, 1]")
    if not 0.0 < args.stride_fraction <= args.window_fraction:
        raise RuntimeError("--stride-fraction must be in (0, window-fraction]")
    if args.min_window_ink_pixels <= 0:
        raise RuntimeError("--min-window-ink-pixels must be positive")
    if args.edge_tolerance < 0:
        raise RuntimeError("--edge-tolerance must be non-negative")

    directory = Path(args.dir)
    reference_pages = numbered_pages(directory, args.reference_prefix)
    candidate_pages = numbered_pages(directory, args.candidate_prefix)
    if not reference_pages:
        raise RuntimeError(f"no reference rasters found in {directory}")
    if len(reference_pages) != len(candidate_pages):
        raise RuntimeError(
            "rendered page counts differ: "
            f"reference={len(reference_pages)}, candidate={len(candidate_pages)}"
        )

    pages: list[dict[str, object]] = []
    for page_number, (reference, candidate) in enumerate(
        zip(reference_pages, candidate_pages), start=1
    ):
        pages.append(
            {
                "page": page_number,
                **compare_windows(
                    reference,
                    candidate,
                    window_fraction=args.window_fraction,
                    stride_fraction=args.stride_fraction,
                    ink_threshold=args.ink_threshold,
                    min_window_ink_pixels=args.min_window_ink_pixels,
                    max_blank_row_gap=args.max_blank_row_gap,
                    edge_tolerance=args.edge_tolerance,
                ),
            }
        )

    minimum_f1 = min(float(page["minimum_window_boundary_f1"]) for page in pages)
    result = {
        "reference_prefix": args.reference_prefix,
        "candidate_prefix": args.candidate_prefix,
        "window_fraction": args.window_fraction,
        "stride_fraction": args.stride_fraction,
        "ink_threshold": args.ink_threshold,
        "min_window_ink_pixels": args.min_window_ink_pixels,
        "edge_tolerance": args.edge_tolerance,
        "minimum_window_boundary_f1": minimum_f1,
        "pages": pages,
    }
    calibration = args.expect_below is not None
    output = Path(args.output) if args.output else directory / (
        "edge-window-calibration.json" if calibration else "edge-window-results.json"
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps(result, indent=2, sort_keys=True))

    if calibration:
        if minimum_f1 >= args.expect_below:
            print(
                "edge-window calibration failed to detect deliberate contour distortion: "
                f"minimum_window_boundary_f1={minimum_f1:.6f}, expected < "
                f"{args.expect_below:.6f}",
                file=sys.stderr,
            )
            return 1
        return 0

    if minimum_f1 < args.min_window_boundary_f1:
        print(
            f"minimum window boundary F1 {minimum_f1:.6f} is below required "
            f"{args.min_window_boundary_f1:.6f}",
            file=sys.stderr,
        )
        return 1
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
