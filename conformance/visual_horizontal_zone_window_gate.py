#!/usr/bin/env python3
"""Gate localized Khmer horizontal mark placement inside upper/lower line zones.

Vertical profiles are sensitive to above/below-base displacement but can miss a mark
that moves sideways while preserving row-wise ink mass. This gate restricts analysis
to a configurable upper or lower line zone, then compares normalized column-wise ink
profiles inside overlapping horizontal windows.
"""
    ...
    *,
    ...
    for line_index, (top, bottom) in enumerate(bands, start=1):
        ...
        for window_index, window_left in enumerate(starts, start=1):
            ...
            reference_profile, reference_ink = normalized_column_profile(
                reference,
                width,
                zone_top,
                zone_bottom,
            ...
            candidate_profile, candidate_ink = normalized_column_profile(
                candidate,
                width,
                zone_top,
                zone_bottom,
    ...
    return {
        ...
        "minimum_horizontal_zone_profile_similarity": min(
...
def main() -> int:
    ...
    minimum_similarity = min(
        float(page["minimum_horizontal_zone_profile_similarity"]) for page in pages
    )
    result = {
        ...
        "ink_threshold": args.ink_threshold,
        "min_zone_ink_pixels": args.min_zone_ink_pixels,
        "minimum_horizontal_zone_profile_similarity": minimum_similarity,
        "pages": pages,
    }
    calibration = args.expect_below is not None
    output = Path(args.output) if args.output else directory / (
        "horizontal-zone-window-calibration.json" if calibration else "horizontal-zone-window-results.json"
    )
    ...
    if calibration:
        if minimum_similarity >= args.expect_below:
            print(
                "localized horizontal mark-zone calibration failed to detect deliberate mark shift: "
    ...
    if minimum_similarity < args.min_zone_profile_similarity:
        print(
            f"minimum local horizontal mark-zone profile similarity {minimum_similarity:.6f} is below "
