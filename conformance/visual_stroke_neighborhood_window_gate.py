#!/usr/bin/env python3
"""Gate localized Khmer grayscale fidelity around rendered stroke neighborhoods.

Binary ink masks and edge-only metrics can miss antialiasing or halo changes just
outside the thresholded stroke. This gate dilates the union of reference and
candidate ink by a small pixel radius, then compares grayscale values inside
that stroke neighborhood using overlapping line-local windows.
"""
    ...
    *,
    ...
    for line_index, (top, bottom) in enumerate(bands, start=1):
        ...
        for window_index, window_left in enumerate(starts, start=1):
            ...
            for x, y in points:
                offset = (y * width + x) * 3
                error = abs(gray(reference, offset) - gray(candidate, offset))
                absolute_error += error
                squared_error += error * error

            mae = absolute_error / len(points)
            rmse = (squared_error / len(points)) ** 0.5
            windows.append(
                {
                    "line": line_index,
                    "window": window_index,
                    ...
                    "rms_similarity": 1.0 - rmse / 255.0,
                }
            )

    if not windows:
        raise RuntimeError("no stroke-neighborhood window met the minimum support threshold")

    return {
        "line_count": len(bands),
        "windows": windows,
        "minimum_similarity": min(float(window["similarity"]) for window in windows),
        "minimum_rms_similarity": min(
            float(window["rms_similarity"]) for window in windows
        ),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-rms-similarity", type=float, default=0.90)
...
def main() -> int:
    ...
    minimum_rms_similarity = min(
        float(page["minimum_rms_similarity"]) for page in pages
    )
    result = {
        ...
        "minimum_rms_similarity": minimum_rms_similarity,
        "pages": pages,
    }
    output = Path(args.output) if args.output else directory / "stroke-neighborhood-results.json"
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps(result, indent=2, sort_keys=True))

    if args.expect_below is not None:
        if minimum_rms_similarity >= args.expect_below:
            print(
                "stroke-neighborhood calibration failed to detect deliberate distortion: "
                f"minimum_rms_similarity={minimum_rms_similarity:.6f}, expected < "
    ...
    if minimum_similarity < args.min_similarity:
        print(
            f"minimum stroke-neighborhood similarity {minimum_similarity:.6f} is below "
            f"required {args.min_similarity:.6f}",
            file=sys.stderr,
        )
        return 1
    if minimum_rms_similarity < args.min_rms_similarity:
        print(
            f"minimum stroke-neighborhood RMS similarity {minimum_rms_similarity:.6f} is below "
            f"required {args.min_rms_similarity:.6f}",
            file=sys.stderr,
        )
        return 1
    return 0
